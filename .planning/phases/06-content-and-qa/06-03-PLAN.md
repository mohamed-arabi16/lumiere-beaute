...
Image optimization (the CONTEXT.md primary remediation) is not applicable — all image slots use CSS gradients with no `<img>` elements. Proceeding to React.lazy() code splitting as the first applicable remediation.
...
**After each remediation level:** Run `npm run build` to confirm no TypeScript errors, then re-run the Lighthouse CLI commands from Task 1. Record the new scores. Phase 6 is blocked until all 5 pages reach ≥ 75.
  ...
  <verify>
    <automated>npm run build 2>&1 | tail -5 && node -e "['home','services','academy','contact','about'].forEach(p=>{ const r=require('./lighthouse-'+p+'.json'); const s=Math.round(r.categories.performance.score*100); if(s<75){console.error(p,s,'FAIL');process.exit(1);} console.log(p,s,'PASS') })"</automated>
